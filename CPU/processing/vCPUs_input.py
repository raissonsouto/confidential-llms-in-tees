...
# Adapted for this reproduction's data -- see vCPUs_batch_size.py for the
# rationale (no vCPU sweep available here, x-axis becomes "system"). This
# variant facets by input length instead of batch size, at batch=1 (the only
# batch size with data across all three input lengths for all three
# systems). The GPU reference line is only drawn where the upstream cost
# table has a matching (input, batch) key; it doesn't cover batch=1 at
# input=512, so that panel is left without a reference line rather than
# guessing.
file = sys.argv[1]
...
GPU_CC_COST = {(128, 1): 13.966463202799583, (1024, 1): 1.817995655649457,
...
fig, ax = plt.subplots(nrows=2, ncols=len(INPUTS), figsize=(10, 5.5))
fig.suptitle(f"batch = {BS}, Llama-2-7B, bf16, {VCPU} vCPUs", y=1.0)

for col, in_size in enumerate(INPUTS):
    sub = df.loc[df['in_size'] == in_size]
    if set(sub['system'].unique()) != set(ORDER):
        ...
        ax[0][col].set_title(f"input = {in_size}")
        continue
    ...
    if key in GPU_CC_COST:
        ...
        ax[1][col].text(0.5, 0.95, "(no GPU reference\nat this input length)", ha='center', va='top',
                         transform=ax[1][col].transAxes, fontsize=8, color='#898781')
    ax[1][col].set_xlabel("")
...
plt.tight_layout()
plt.savefig("../../results/vCPUs_GPU_EMR_inputs.png", bbox_inches='tight', transparent=True, pad_inches=0, dpi=150)
print("wrote ../../results/vCPUs_GPU_EMR_inputs.png")
